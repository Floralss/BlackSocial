...
document.getElementById("voiceBtn")?.addEventListener("click", async () => {
  ...
  try {
    ...
    mediaRecorder = mime
      ? new MediaRecorder(voiceStream, { mimeType: mime, audioBitsPerSecond: 64000 })
...
async function sendVoiceMessage(blob) {
  ...
  if (!blob || blob.size < 100) return showToast("Пустая запись");
  setLoading(true);
  showToast("Отправка голосового…");
  try {
    let audioUrl = "";
    // Prefer Firebase Storage (не упирается в лимит Firestore)
    try {
      ...
      console.warn("storage voice fail, fallback dataUrl", stErr);
      // fallback: short dataUrl only if small
      ...
      if (typeof dataUrl === "string" && dataUrl.length > 700000) {
        throw new Error("Нет доступа к Storage и файл слишком большой. Включи Firebase Storage rules.");
      }
      audioUrl = dataUrl;
    }
    await addDoc(collection(db, "chats", currentChatId, "messages"), {
