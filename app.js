...
let selectedPayMethod = "blackvault";
...
function setAvatarEl(node, profile) {
  ...
  node.innerHTML = "";
  node.textContent = "";
  node.style.background = hashColor(profile.name || profile.username || "?");
  if (profile.avatarUrl) {
    ...
    node.textContent = initials(profile.name || profile.username || "?");
  }
}
async function fillAvatarByUid(node, uid, fallbackName) {
  if (!node || !uid) return;
  if (avatarCache[uid]) {
    setAvatarEl(node, avatarCache[uid]);
    return;
  }
  node.style.background = hashColor(fallbackName || "?");
  node.textContent = initials(fallbackName || "?");
  try {
    const s = await getDoc(doc(db, "users", uid));
    if (s.exists()) {
      avatarCache[uid] = { uid, ...s.data() };
      setAvatarEl(node, avatarCache[uid]);
    }
  } catch (_) {}
}
...
function renderSettingsView() {
  ...
  if (settingsView === "privacy") {
    ...
    body.innerHTML = `
      ...
      <div class="settings-section">
        ...
        <div class="settings-row"><span>Платные сообщения</span>
          <button type="button" class="toggle ${(s.paidMessages || currentProfile.msgPrice > 0) ? "on" : ""}" data-key="paidMessages" id="paidMsgToggle"></button>
        </div>
        <div class="settings-row" id="msgPriceRow">
          <span>Цена за 1 сообщение (BC)</span>
          <input type="number" id="msgPriceInput" class="settings-select" min="0" step="1"
            value="${Number(currentProfile.msgPrice || s.msgPrice || 0)}" style="width:90px;text-align:right">
        </div>
        <p style="font-size:12px;color:var(--text-2);padding:0 4px 8px;line-height:1.4">
          Если цена ≥ 10 BC, первый (и каждое) сообщение от другого человека спишет у него монеты и зачислит тебе.
          0 = бесплатно. Минимум для платных — 10.
        </p>
  ...
  const priceInp = document.getElementById("msgPriceInput");
  const savePrice = async () => {
    let price = parseInt(priceInp?.value || "0", 10);
    if (isNaN(price) || price < 0) price = 0;
    if (price > 0 && price < 10) {
      showToast("Минимум 10 BC или 0 (бесплатно)");
      price = 0;
      if (priceInp) priceInp.value = "0";
    }
    const settings = { ...(currentProfile.settings || {}), msgPrice: price, paidMessages: price > 0 };
    try {
      await updateDoc(doc(db, "users", currentUser.uid), { msgPrice: price, settings });
      currentProfile.msgPrice = price;
      currentProfile.settings = settings;
      showToast(price > 0 ? `Платные сообщения: ${price} BC` : "Сообщения бесплатные");
    } catch (e) { showToast(translateAuthError(e)); }
  };
  priceInp?.addEventListener("change", savePrice);
  document.getElementById("paidMsgToggle")?.addEventListener("click", async () => {
    // after toggle handler runs, sync price
    setTimeout(async () => {
      const on = document.getElementById("paidMsgToggle")?.classList.contains("on");
      if (!on) {
        if (priceInp) priceInp.value = "0";
        await savePrice();
      } else {
        if (priceInp && parseInt(priceInp.value, 10) < 10) priceInp.value = "10";
        await savePrice();
      }
    }, 50);
  });
...
function openShop() {
  ...
  selectedPayCur = "USD";
  selectedPayMethod = selectedPayMethod || "blackvault";
...
function updatePayMethodUI() {
  document.querySelectorAll(".pay-method").forEach(b => {
    b.classList.toggle("sel", b.dataset.pay === selectedPayMethod);
  });
  const box = document.getElementById("vaultLoginBox");
  if (box) box.classList.toggle("hidden", selectedPayMethod !== "blackvault");
}
...
function updatePaySummary() {
  ...
  const pack = selectedCoinPack || { coins: 100, uah: 50 };
  const cur = selectedPayCur || "USD";
  const method = selectedPayMethod === "google" ? "Google Pay" : "BlackVault";
  el.textContent = `${pack.coins} BC · ${formatPackMoney(pack, cur)} · ${method}`;
}

document.getElementById("payMethods")?.addEventListener("click", (e) => {
  const btn = e.target.closest(".pay-method");
  if (!btn) return;
  selectedPayMethod = btn.dataset.pay || "blackvault";
  updatePayMethodUI();
  updatePaySummary();
});

async function payWithBlackVault(pack, cur) {
  const email = document.getElementById("vaultEmail")?.value.trim() || "";
  const pass = document.getElementById("vaultPassword")?.value || "";
  if (!email || !pass) {
    document.getElementById("vaultLoginBox")?.classList.remove("hidden");
    throw new Error("Введи email и пароль BlackVault");
  }
  ...
  if (document.getElementById("vaultPassword")) document.getElementById("vaultPassword").value = "";
}

async function payWithGooglePay(pack, cur) {
  const amount = packMoney(pack, cur);
  const currency = cur === "UAH" ? "UAH" : "USD";
  const value = amount.toFixed(2);
  if (!window.PaymentRequest) {
    throw new Error("Google Pay / Payment Request не поддерживается этим браузером");
  }
  const methodData = [{
  ...
  if (!can) {
    // Fallback: open Google Pay site / explain
    const ok = confirm(`Оплата ${value} ${currency} за ${pack.coins} BC через Google Pay.\n\nНажми OK, чтобы продолжить (тестовый режим браузера).`);
    if (!ok) throw new Error("Оплата отменена");
    return; // allow credit in demo if user confirms
  }
...
document.getElementById("buyCoinsBtn").addEventListener("click", async () => {
  ...
  setLoading(true);
  try {
    if (selectedPayMethod === "google") {
      await payWithGooglePay(pack, cur);
    } else {
      await payWithBlackVault(pack, cur);
    }
    ...
    updateDrawer();
    showToast(`Оплачено · +${pack.coins} BC`);
  } catch (err) {
    console.error(err);
    ...
    try {
      ...
      try {
        allChats.forEach(c => {
          ...
          showPhoneNotification(chatDisplayName(c), c.lastMessage || "Новое сообщение", c.id);
        });
      } catch (ne) { console.warn("notify", ne); }
      renderChatList();
    } catch (e) {
      console.error("subscribeChats", e);
      try { renderChatList(); } catch (_) {}
    }
  }, err => {
    console.error(err);
    showToast(translateAuthError(err) || "Ошибка списка чатов");
  });
}
function chatDisplayName(chat) {
  ...
  try {
    if (chat.type === "private") {
      ...
      if (other && chat.memberNames && chat.memberNames[other]) return chat.memberNames[other];
      if (other && avatarCache[other]?.name) return avatarCache[other].name;
      return "Пользователь";
    }
...
function renderChatList() {
  ...
  try {
    filtered = (allChats || []).filter(c => {
      ...
      const prev = (c.lastMessage || "").toLowerCase();
      return name.includes(filter) || prev.includes(filter);
    });
  } catch (e) {
    console.error("filter chats", e);
    filtered = allChats || [];
  }
  if (!filtered.length) {
    listEl.innerHTML = `<div class="search-empty">${(allChats || []).length ? "Ничего не найдено по поиску" : "Нет чатов · ☰ → Новый чат"}</div>`;
    return;
  }
  filtered.forEach(c => {
    ...
    const item = el(`
      <div class="chat-item" data-id="${c.id}">
        <div class="chat-avatar" data-av></div>
        <div class="chat-meta">
    ...
    const av = item.querySelector("[data-av]");
    if (c.type === "private") {
      const other = (c.members || []).find(m => m !== currentUser.uid);
      fillAvatarByUid(av, other, name);
    } else {
...
function openChat(chatId) {
  ...
  if (currentChat.type === "private") {
    const other = (currentChat.members || []).find(m => m !== currentUser.uid);
    fillAvatarByUid(hAv, other, name);
  } else {
...
async function chargeIfPaidMessage(chat) {
  if (!chat || chat.type !== "private") return true;
  const otherUid = (chat.members || []).find(m => m !== currentUser.uid);
  if (!otherUid) return true;
  // contacts / premium exception: free if peer is in my contacts? No — pay if THEY set price
  let peer;
  try {
    if (avatarCache[otherUid]) peer = avatarCache[otherUid];
    else {
      const s = await getDoc(doc(db, "users", otherUid));
      if (s.exists()) { peer = { uid: otherUid, ...s.data() }; avatarCache[otherUid] = peer; }
    }
  } catch (_) {}
  const price = Number(peer?.msgPrice || peer?.settings?.msgPrice || 0);
  if (!price || price < 10) return true;
  if ((currentProfile.blackCoins || 0) < price) {
    showToast(`Сообщение стоит ${price} BC — недостаточно монет`);
    return false;
  }
  if (!confirm(`Сообщение для ${peer.name || "пользователя"} стоит ${price} BC. Оплатить?`)) return false;
  try {
    await runTransaction(db, async (tx) => {
      const meRef = doc(db, "users", currentUser.uid);
      const peerRef = doc(db, "users", otherUid);
      const meSnap = await tx.get(meRef);
      const peerSnap = await tx.get(peerRef);
      const bal = meSnap.data()?.blackCoins || 0;
      if (bal < price) throw new Error("Недостаточно BC");
      tx.update(meRef, { blackCoins: bal - price });
      tx.update(peerRef, { blackCoins: (peerSnap.data()?.blackCoins || 0) + price });
    });
    ...
    // peer update may fail due to rules — still debit self and write transfer
    try {
      await updateDoc(doc(db, "users", currentUser.uid), { blackCoins: increment(-price) });
      currentProfile.blackCoins = (currentProfile.blackCoins || 0) - price;
      await writeTransfer({ type: "coins", toUid: otherUid, amount: price, reason: "paid_message" });
      updateDrawer();
      showToast(`−${price} BC (перевод)`);
      return true;
    } catch (e2) {
      showToast(e2.message || "Оплата не прошла");
      return false;
    }
  }
}
...
async function sendMessage() {
  ...
  try {
    const okPay = await chargeIfPaidMessage(currentChat);
    if (!okPay) return;
...
function pickVoiceMime() {
  ...
  for (const tp of types) {
    try { if (window.MediaRecorder && MediaRecorder.isTypeSupported(tp)) return tp; } catch (_) {}
  }
  return "";
}
...
document.getElementById("voiceBtn")?.addEventListener("click", async () => {
  ...
  if (mediaRecorder && mediaRecorder.state === "recording") {
    // second click = stop & send
    try { mediaRecorder.requestData?.(); } catch (_) {}
    try { mediaRecorder.stop(); } catch (_) {}
    return;
  }
  if (!navigator.mediaDevices?.getUserMedia) {
    return showPermissionHelp("Браузер не поддерживает запись звука");
  }
  if (!isSecureForMedia()) {
    return showPermissionHelp("Нужен HTTPS для микрофона");
  }
  try {
    ...
    if (name === "NotAllowedError" || name === "PermissionDeniedError") {
      showPermissionHelp("Микрофон заблокирован.");
    } else {
      showPermissionHelp("Микрофон: " + (e.message || name));
    }
    return;
  }
  try {
    ...
    mediaRecorder = mime
      ? new MediaRecorder(voiceStream, { mimeType: mime, audioBitsPerSecond: 24000 })
      : new MediaRecorder(voiceStream);
    mediaRecorder.ondataavailable = (e) => {
      if (e.data && e.data.size > 0) voiceChunks.push(e.data);
    };
    mediaRecorder.onerror = () => showToast("Ошибка записи");
    mediaRecorder.onstop = async () => {
      ...
      if (!voiceChunks.length) {
        showToast("Пустая запись — говори громче и держи 1–2 сек");
        return;
      }
      const blob = new Blob(voiceChunks, { type });
      voiceChunks = [];
      if (blob.size < 200) return showToast("Слишком коротко");
      if (blob.size > 4 * 1024 * 1024) return showToast("Слишком длинное");
    ...
    showToast("Идёт запись… нажми 🎤 ещё раз или «Отправить»");
...
async function sendVoiceMessage(blob) {
  if (!currentChatId) return showToast("Нет чата");
  if (!blob || blob.size < 80) return showToast("Пустая запись — говори дольше");
  const okPay = await chargeIfPaidMessage(currentChat);
  if (!okPay) return;
  setLoading(true);
  try {
    ...
    // Small enough → Firestore (работает без Storage)
    if (typeof dataUrl === "string" && dataUrl.length <= 600000) {
      audioUrl = dataUrl;
    } else {
      try {
        const ext = (blob.type || "").includes("mp4") ? "mp4" : "webm";
        const path = `voice/${currentUser.uid}/${Date.now()}.${ext}`;
        await uploadBytes(ref(storage, path), blob, { contentType: blob.type || "audio/webm" });
        audioUrl = await getDownloadURL(ref(storage, path));
      } catch (stErr) {
        console.error(stErr);
        throw new Error("ГС слишком длинное и Storage недоступен. Запиши короче (до 15 сек) или включи Storage Rules в Firebase.");
      }
    }
    await addDoc(collection(db, "chats", currentChatId, "messages"), {
      ...
      audioUrl,
      mediaType: "voice",
      duration: Math.max(1, Math.round((Date.now() - (voiceStartTs || Date.now())) / 1000)),
      timestamp: serverTimestamp()
    });
    await updateDoc(doc(db, "chats", currentChatId), {
    ...
    console.error(e);
    showToast(e.message || translateAuthError(e) || "Не удалось отправить ГС");
  } finally { setLoading(false); }
}
...
async function startCall() {
  ...
  const otherUid = (currentChat.members || []).find(m => m !== currentUser.uid);
  if (!otherUid) return showToast("Собеседник не найден");
  const name = chatDisplayName(currentChat);
  try {
  ...
  try {
    ...
    console.error(e);
    showToast("Звонок не удался: " + (e.message || "проверь Firestore rules для calls"));
    endCall(false);
  }
}
...
function showCallUI(name, otherUid, isCaller) {
  ...
  box = el(`<div class="call-overlay" id="callOverlay">
    <div class="call-stage">
      <div class="call-avatar-lg" id="callAv" style="background:${hashColor(name)}">${initials(name)}</div>
      <div class="call-name">${escapeHtml(name)}</div>
      <div class="call-status" id="callStatus">${isCaller ? "ожидание…" : "входящий…"}</div>
      <audio id="remoteAudio" autoplay playsinline></audio>
      <div class="call-bar">
        <button type="button" class="call-round" id="callMuteBtn" title="Микрофон">🔇</button>
        ${!isCaller ? '<button type="button" class="call-round call-accept" id="callAcceptBtn" title="Ответить">✅</button>' : ""}
        <button type="button" class="call-round call-end" id="callEndBtn" title="Завершить">📞</button>
        <button type="button" class="call-round" id="callSpeakerBtn" title="Динамик">🔊</button>
      </div>
      <div class="call-bar-labels">
        <span>Микрофон</span>
        ${!isCaller ? "<span>Ответить</span>" : ""}
        <span>Сброс</span>
        <span>Звук</span>
      </div>
    </div>
  ...
  document.body.appendChild(box);
  if (otherUid) fillAvatarByUid(document.getElementById("callAv"), otherUid, name);
  let muted = false;
  document.getElementById("callMuteBtn").onclick = () => {
    ...
    showToast(muted ? "Микрофон выкл" : "Микрофон вкл");
  };
  document.getElementById("callSpeakerBtn").onclick = () => {
    const a = document.getElementById("remoteAudio");
    if (a) { a.muted = !a.muted; showToast(a.muted ? "Звук выкл" : "Звук вкл"); }
  };
