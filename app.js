...
import {
  ...
  query, where, orderBy, limit, onSnapshot, getDocs, serverTimestamp, increment, arrayUnion, arrayRemove
...
document.getElementById("closeContacts")?.addEventListener("click", () => document.getElementById("contactsScreen").classList.add("hidden"));
document.getElementById("closeNft")?.addEventListener("click", () => document.getElementById("nftScreen").classList.add("hidden"));
document.getElementById("closeChatInfo")?.addEventListener("click", () => document.getElementById("chatInfoScreen").classList.add("hidden"));
document.getElementById("closeUserProfile")?.addEventListener("click", () => document.getElementById("userProfileScreen").classList.add("hidden"));
...
function openSettings() {
  ...
  const th = localStorage.getItem("bs_theme") || currentProfile.settings?.theme || "dark";
  applyTheme(th);
}
...
async function openPeerProfile() {
  ...
  try {
    ...
    const isContact = contacts.includes(otherUid);
    const body = document.getElementById("userProfileBody");
    body.innerHTML = "";
    const av = el(`<div class="profile-avatar-wrap"><div class="profile-avatar-big" id="peerAv"></div>
      <div style="font-size:18px;font-weight:700">${escapeHtml(u.name)}</div>
      <div style="color:var(--accent-hover)">@${escapeHtml(u.username)}</div>
      ${hasPremium(u) ? '<span class="premium-badge">PREMIUM</span>' : ""}
    </div>`);
    body.appendChild(av);
    ...
    document.getElementById("toggleContactBtn").onclick = async () => {
      try {
        if (isContact) {
          await updateDoc(doc(db, "users", currentUser.uid), { contacts: arrayRemove(otherUid) });
          currentProfile.contacts = (currentProfile.contacts || []).filter(x => x !== otherUid);
          showToast("Удалён из контактов");
        } else {
          await updateDoc(doc(db, "users", currentUser.uid), { contacts: arrayUnion(otherUid) });
          currentProfile.contacts = [...(currentProfile.contacts || []), otherUid];
...
async function openChatInfo() {
  if (!currentChat) return;
  hideAllScreens();
  document.getElementById("chatInfoScreen").classList.remove("hidden");
  ...
  let html = `
    <div class="profile-avatar-wrap">
      <div class="profile-avatar-big" style="background:${hashColor(currentChat.name)}">${initials(currentChat.name)}</div>
      <div style="font-size:18px;font-weight:700">${escapeHtml(currentChat.name || "")}</div>
      <div style="color:var(--text-2);font-size:13px">${currentChat.type === "channel" ? "Канал" : "Группа"} · ${members.length} участн.</div>
    </div>`;
  ...
  if (isAdm) {
    ...
    <div class="settings-section"><h3>Настройки</h3>
      ...
      <div class="settings-row"><span>Публичный (в поиске)</span><button type="button" class="toggle ${currentChat.public ? "on" : ""}" id="ciPublic"></button></div>
  ...
  document.getElementById("ciPublic")?.addEventListener("click", function() { this.classList.toggle("on"); });
  ...
  document.getElementById("ciSave")?.addEventListener("click", async () => {
    try {
      await updateDoc(doc(db, "chats", currentChatId), {
        ...
        public: document.getElementById("ciPublic")?.classList.contains("on") || false,
...
async function openNftScreen() {
  hideAllScreens();
  document.getElementById("nftScreen").classList.remove("hidden");
  await renderNfts();
}
...
function nftCardEl(n, owned, canBuy) {
  ...
  const artStyle = n.imageUrl ? "" : `background:${n.bg || "var(--accent)"}`;
  const artInner = n.imageUrl
    ? `<img src="${escapeHtml(n.imageUrl)}" alt="">`
    : escapeHtml(n.emoji || "◆");
  const card = el(`<div class="nft-card">
    <div class="nft-art" style="${artStyle}"><span class="nft-rare">#${n.nftId || "?"} · ${rare}</span>${artInner}</div>
    <div class="nft-meta">
      <div class="nft-name">${escapeHtml(n.name || "NFT")}</div>
      <div class="nft-id">${owned ? "В коллекции" : `осталось ${n.remaining ?? n.supply ?? 0}`}</div>
      ${canBuy ? `<div class="nft-price">${n.price} BC · купить</div>` : ""}
    </div>
  </div>`);
  if (canBuy) {
    card.onclick = () => buyNft(n);
  }
  return card;
}
...
document.getElementById("nftCreateBtn")?.addEventListener("click", async () => {
  ...
  try {
    ...
    const ref = await addDoc(collection(db, "nfts"), {
      ...
      imageUrl: document.getElementById("nftImage").value.trim() || "",
