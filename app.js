...
async function renderUserProfile(u, isPeer) {
  ...
  const online = u.settings?.showOnline !== false;
  ...
  const head = el(`<div class="tg-profile">
    ...
    <div class="tg-status">${online ? "в сети" : "@" + escapeHtml(u.username || "")}</div>
  </div>`);
  ...
  const giftsSec = el(`<div class="tg-section">
    <div class="tg-gifts-head"><span>🎁 NFT</span><span style="color:var(--text-2);font-weight:500">${nfts.length}</span></div>
    <div class="tg-gifts-row" id="peerGifts"></div>
  </div>`);
  body.appendChild(giftsSec);
  const row = document.getElementById("peerGifts");
  if (!nfts.length) {
    row.innerHTML = '<span style="color:var(--text-2);font-size:13px;padding:4px 0">Пока нет NFT</span>';
  } else {
