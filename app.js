...
document.getElementById("avatarInput").addEventListener("change", async (e) => {
  ...
  try {
    ...
    try {
      ...
      console.warn("avatar storage fail", e);
      url = await fileToDataUrl(file, 512, 0.7);
      if (url.length > 900000) throw new Error("Аватар слишком большой");
    }
...
document.getElementById("closeSettings").addEventListener("click", () => { document.getElementById("settingsScreen").classList.add("hidden"); document.getElementById("app")?.classList.remove("overlay-open"); });
document.querySelectorAll(".toggle[data-key]").forEach(btn => {
...
async function uploadImage(file, chatId) {
  ...
  try {
    ...
    console.warn("Storage fail, base64 fallback", e);
  }
  // 2) base64 fallback (без Storage)
  const dataUrl = await fileToDataUrl(file);
  if (dataUrl.length > 900000) throw new Error("Файл слишком большой для отправки без Storage");
  return dataUrl;
}
