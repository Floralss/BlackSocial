...
async function sendGiftToUser(toUser) {
  if (!hasPremium(currentProfile) && false) { /* optional lock */ }
  setLoading(true);
  try {
    const cat = await getDocs(query(collection(db, "giftCatalog"), limit(30)));
    ...
    const gift = {
      catalogId: pick.id, name: pick.name, emoji: pick.emoji, bg: pick.bg,
